    ...
    var body: some View {
        TabView(selection: self.$selection.tab) {
            ...
            ProvidersPane(settings: self.settings, store: self.store)
                .tabItem { Label(L10n.tabProviders, systemImage: "square.grid.2x2") }
                .tag(PreferencesTab.providers)

            DisplayPane(settings: self.settings, store: self.store)
                .tabItem { Label(L10n.tabDisplay, systemImage: "eye") }
            ...
            if self.settings.debugMenuEnabled {
                DebugPane(settings: self.settings, store: self.store)
                    .tabItem { Label(L10n.tabDebug, systemImage: "ladybug") }
